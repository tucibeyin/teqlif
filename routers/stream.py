...
async def read_live(request: Request, mode: str = "watch", broadcaster: str = None, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Broadcaster verisini doğru çek
    target_user = None
    if broadcaster:
    ...
    # Eğer yayıncı modundaysa target_user kendisidir
...
async def broadcast(websocket: WebSocket, db: Session = Depends(get_db)):
    ...
    print(f"🎥 YAYIN (PREMIUM STABLE): {user.username}")

    command = [
        "ffmpeg", 
        # GİRİŞ: WebM (En uyumlusu)
        "-f", "webm", 
        ...
        # PERFORMANS (Veryfast = Güvenli)
        "-preset", "veryfast", "-tune", "zerolatency", "-threads", "0", 
        "-af", "aresample=async=1",
        
        # iOS UYUMLULUK (Baseline + Zorla Keyframe)
        "-profile:v", "baseline", "-level", "3.0", 
        "-g", "60", "-keyint_min", "60", "-sc_threshold", "0", 
        "-force_key_frames", "expr:gte(t,n_forced*2)", 
        "-pix_fmt", "yuv420p",

        # ÇIKTI (Düşük/Orta Bitrate = Akıcılık)
    ...
    # 30 Saniye Bekleme Döngüsü (Güvenli)
