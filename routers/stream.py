...
def video_generator(filepath):
    ...
    with open(filepath, "rb") as f:
        while True:
            ...
            if not data:
                time.sleep(0.1) # Veri yoksa bekle (Canlı yayın sürüyor)
                continue
            yield data
...
async def stream_video(username: str):
    ...
    # Tarayıcıya bunun bir video akışı olduğunu söyle
...
async def broadcast(websocket: WebSocket, db: Session = Depends(get_db)):
    ...
    print(f"🎥 YAYIN BAŞLIYOR (RAW FILE WRITER): {user.username}")
