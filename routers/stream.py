...
async def broadcast_endpoint(websocket: WebSocket, db: Session = Depends(get_db)):
    ...
    print(f"🎥 YAYIN BAŞLIYOR (ASPECT RATIO FIX): {user.username}")

    # 🔥 DÜZELTME: scale=-2:720 -> En boy oranını koru, yüksekliği 720 yap, genişlik çift sayı olsun.
    command = [
        ...
        "[v720]scale=-2:720[out720];"  # Yükseklik 720, Genişlik otomatik (Oranı bozma)
