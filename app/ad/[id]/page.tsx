...
import HybridLiveWrapper from "./HybridLiveWrapper";
...
export default async function AdDetailPage({
    ...
    // 5. Component Swapping Logistics: If isLive is true, we wrap the whole page (or relevant sections) 
    // in HybridLiveWrapper so the LiveKit Room context is available everywhere inside.
    const Wrapper = adData.isLive ? HybridLiveWrapper : React.Fragment;

    return (
        // @ts-ignore
        <Wrapper {...(adData.isLive ? { roomId: adData.id, isOwner } : {})}>
            <div className="container" style={{ paddingTop: adData.isLive ? "0.5rem" : "0" }}>
                {adData.isLive && (
                    <div style={{ background: "#ef4444", color: "white", padding: "0.5rem 1rem", fontWeight: "bold", display: "flex", alignItems: "center", gap: "0.5rem", borderRadius: "1rem", marginBottom: "1rem" }}>
                ...
                <div className="ad-detail">
                    ...
                    <div>
                        <div className="ad-detail-images">
                            {adData.isLive ? (
                                <LiveVideoView isOwner={isOwner} adOwnerName={displayName} adId={adData.id} />
                        ...
                        {adData.isFixedPrice ? (
                            ...
                            <div className="auction-card">
                                ...
                                <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                                    {ad.status === 'ACTIVE' && session?.user && adData.isLive && (
                                        <LiveBidConsole
                                            adId={ad.id}
                                            isOwner={isOwner}
                                            initialPrice={highestBid?.amount || ad.startingBid || 0}
