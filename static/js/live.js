document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 1. SOHBET & UI (Eski kodlarınla aynı, kısalttım) ---
    // (Buraya UI fonksiyonlarını koyabilirsin veya önceki dosyadakileri koru)
    ...
    // --- 3. İZLEYİCİ (CACHE-BUSTING & RECOVERY) ---
    else if (MODE === 'watch' && CONFIG.broadcaster) {
        const u = CONFIG.broadcaster;
        const v = document.getElementById(`video-${u}`);
        if (v) {
            // 🔥 CACHE ÖNLEYİCİ PARAMETRE 🔥
            ...
            if (Hls.isSupported()) {
                const hls = new Hls({
                    ...
                    maxBufferLength: 10,
                    manifestLoadingTimeOut: 15000, // Manifest yükleme süresini uzat
                });
