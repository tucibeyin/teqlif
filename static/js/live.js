document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 2. GÖRÜNÜM GÜNCELLEME (Fiyat & Lider) ---
    function updatePriceDisplay(amount, target, bidderName) {
        // Yayıncı ve İzleyici için ID kontrolü
        ...
        fetch('/gift/send', { method: 'POST', body: formData })
            ...
            .then(data => {
                if (data.status === 'success') {
                    // Tüm elmas sayaçlarını güncelle
                    document.querySelectorAll('.info-pill.diamond span, #menu-diamond-count, #screen-diamond-count')
                        .forEach(el => el.innerText = data.new_balance);
                    closeGiftMenu();
                } else {
                    alert(data.msg);
                }
    ...
    if (MODE === 'broadcast') {
        ...
        async function initStream() {
            try {
                // Kamera Ayarları (Android için facingMode eklendi)
            ...
            startBtn.addEventListener('click', function () {
                ...
                fetch('/broadcast/start', { method: 'POST', body: formData })
                    ...
                    .then(data => {
                        ...
                        ws.onopen = () => {
                            // 🔥 ANDROID & CHROME FORMAT SEÇİCİ 🔥
                            let mimeType = 'video/webm;codecs=vp9';
                            if (!MediaRecorder.isTypeSupported(mimeType)) {
                                mimeType = 'video/webm;codecs=vp8'; // Android genelde bunu sever
                                if (!MediaRecorder.isTypeSupported(mimeType)) {
                                    mimeType = 'video/webm;codecs=h264';
                                    if (!MediaRecorder.isTypeSupported(mimeType)) {
                                        mimeType = 'video/webm'; // Son çare
                                    }
                                }
                            }

                            console.log("Seçilen Video Formatı:", mimeType);
    ...
    window.unmuteVideo = function (u) {
        ...
        if (v) {
            ...
            v.parentElement.querySelector('.tap-hint').style.display = 'none';
        }
    }
        ...
        fetch('/user/follow', { method: 'POST', body: formData })
            ...
            .then(data => {
                if (data.status === 'followed') {
                    if (btn) { btn.classList.add('following'); btn.innerText = '✓'; }
                } else {
                    if (btn) { btn.classList.remove('following'); btn.innerText = '+'; }
                }
            });
    }
