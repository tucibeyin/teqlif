// Global değişkenleri ve ayarları window.TEQLIF_CONFIG üzerinden alacağız
...
function connectChat(target) {
    ...
    // Broadcast modunda kullanıcı adı config'den, izleme modunda target parametresinden gelir
    ...
    ws.onmessage = (e) => {
        ...
        if (d.type === 'auction_state') {
            ...
            // Broadcast modunda ID'ler farklı olabilir, kontrol et
...
if (MODE === 'broadcast') {
    ...
    async function initStream(audioDeviceId = null) {
        ...
        try {
            ...
            console.error(err);
            alert("Kamera/Mikrofon Hatası! (İzinleri Kontrol Et)");
        }
    }

    // Sayfa yüklenince kamerayı başlat
    initStream();
    ...
    window.startBroadcast = function () {
        ...
        ws.onopen = () => {
            ...
            // 1 saniyelik HLS segmentleri için 500ms chunk ideal
            rec.start(500);
