                            ...
                            <a href="${isOwner ? '/hesabim.html' : '/profil/' + esc(data.user.username)}" class="seller-row">
                                ...
                                <div style="flex:1;min-width:0;">
                                    <div style="display:flex;align-items:center;flex-wrap:wrap;gap:4px;margin-bottom:2px;">
                                        <span class="seller-name" style="margin:0;">${esc(data.user.full_name || data.user.username)}</span>
                                        ${data.user.is_verified ? `<span style="display:inline-flex;align-items:center;gap:3px;background:#2563eb;color:#fff;font-size:.65rem;font-weight:700;padding:1px 5px;border-radius:10px;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" style="width:8px;height:8px;fill:white;"><path d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM369 209L241 337c-9.4 9.4-24.6 9.4-33.9 0l-64-64c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l47 47L335 175c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9z"/></svg>Onaylı</span>` : ''}
                                        ${data.user.is_premium ? `<span style="display:inline-flex;align-items:center;gap:3px;background:linear-gradient(to right,#0891b2,#06b6d4);color:#fff;font-size:.65rem;font-weight:700;padding:1px 5px;border-radius:10px;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" style="width:8px;height:8px;fill:white;"><path d="M309 106c11.4-7 19-19.7 19-34 0-22.1-17.9-40-40-40s-40 17.9-40 40c0 14.4 7.6 27 19 34L209.7 220.6c-9.1 18.2-32.7 23.4-48.6 10.7L72 160c5-6.7 8-15 8-24 0-22.1-17.9-40-40-40S0 113.9 0 136s17.9 40 40 40l.7 0L86.4 427.4c5.5 30.4 32 52.6 63.6 52.6l276 0c31.6 0 58.1-22.2 63.6-52.6L535.3 176l.7 0c22.1 0 40-17.9 40-40s-17.9-40-40-40-40 17.9-40 40c0 9 3 17.3 8 24l-89.1 71.3c-15.9 12.7-39.5 7.5-48.6-10.7L309 106z"/></svg>PRO</span>` : ''}
                                        ${data.user.seller_badge === 'trusted_seller' ? `<span style="display:inline-flex;align-items:center;gap:3px;background:#16a34a;color:#fff;font-size:.65rem;font-weight:700;padding:1px 5px;border-radius:10px;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" style="width:8px;height:8px;fill:white;"><path d="M256 0c4.6 0 9.2 1 13.4 2.9L457.7 82.8c22 9.3 38.4 31 38.3 57.2-.5 99.2-41.3 280.7-213.6 363.2-16.7 8-36.1 8-52.8 0C57.3 420.7 16.5 239.2 16 140c-.1-26.2 16.3-47.9 38.3-57.2L242.7 2.9C246.8 1 251.4 0 256 0z"/></svg>Güvenilir</span>` : data.user.seller_badge === 'active_seller' ? `<span style="display:inline-flex;align-items:center;gap:3px;background:#f59e0b;color:#fff;font-size:.65rem;font-weight:700;padding:1px 5px;border-radius:10px;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" style="width:8px;height:8px;fill:white;"><path d="M349.4 44.6c5.9-13.7 1.5-29.7-10.6-38.5s-28.6-8-39.9 1.8l-256 224c-10 8.8-13.6 22.9-8.9 35.3S50.7 288 64 288l111.5 0L98.6 467.4c-5.9 13.7-1.5 29.7 10.6 38.5s28.6 8 39.9-1.8l256-224c10-8.8 13.6-22.9 8.9-35.3s-16.6-20.7-30-20.7l-111.5 0L349.4 44.6z"/></svg>Aktif</span>` : ''}
                                    </div>
    ...
    async function _runBoostCampaign() {
        ...
        try {
            var result = await apiFetch('/ads/campaigns', {
                method: 'POST',
                body: JSON.stringify({ listing_id: listingId, total_budget: 50, cpc_bid: 1 })
            });
