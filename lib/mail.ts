...
export async function sendProfileUpdateVerificationEmail(to: string, code: string) {
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #eee; border-radius: 8px; overflow: hidden;">
      ...
      <div style="padding: 30px; text-align: center; color: #333;">
        <h2 style="color: #008FA3;">Profil Güncelleme İşlemi</h2>
        <p style="font-size: 16px; line-height: 1.5;">Hesap bilgilerinizde değişiklik yapmak üzere bir talepte bulundunuz. Bu işlemi onaylamak için lütfen aşağıdaki 6 haneli güvenlik kodunu uygulamaya girin:</p>
        <div style="background-color: #f4f7fa; padding: 20px; margin: 30px 0; border-radius: 8px; border: 1px dashed #00b4cc;">
          <span style="font-size: 32px; font-weight: bold; color: #00B4CC; letter-spacing: 5px;">${code}</span>
        </div>
        <p style="font-size: 14px; color: #777;">Bu kod 15 dakika boyunca geçerlidir. Eğer bu işlemi siz yapmadıysanız lütfen şifrenizi değiştirin.</p>
      </div>
      <div style="background-color: #f9f9f9; padding: 15px; text-align: center; font-size: 12px; color: #aaa;">
        © 2026 teqlif. Tüm hakları saklıdır.
      </div>
