...
## `[ ]` Adım 1: Nginx Gateway (Cloudflare Real IP) Düzeltmesi
...
- VPS Adımı: Node 4 (Core) üzerinde `systemctl restart teqlif.service` çalıştırılması.
...
- VPS Adımı (Adım 3 ve 4): Node 4 (Core) üzerinde `systemctl restart teqlif.service` yapılması ve testlerin (eğer varsa) koşturulması.
