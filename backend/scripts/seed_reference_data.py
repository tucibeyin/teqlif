...
_DISTRICTS: dict[str, list[str]] = {
    ...
    "Afyonkarahisar": ["Başmakçı","Bayat","Bolvadin","Çay","Çobanlar","Dinar","Emirdağ","Evciler","Hocalar","İhsaniye","İscehisar","Kızılören","Merkez","Sandıklı","Sinanpaşa","Sultandağı","Şuhut"],
    ...
    "Samsun": ["Alaçam","Asarcık","Atakum","Ayvacık","Bafra","Çarşamba","Havza","İlkadım","Kavak","Ladik","Ondokuzmayıs","Salıpazarı","Tekkeköy","Terme","Vezirköprü","Yakakent"],
...
async def seed():
    async with AsyncSessionLocal() as db:
        ...
        # ── Districts ──────────────────────────────────────────────────────────
        dist_inserted = 0
        for province, districts in _DISTRICTS.items():
            ...
            for district in districts:
                result = await db.execute(
                    text(
                        "INSERT INTO districts (city_id, name) VALUES (:c, :n) "
