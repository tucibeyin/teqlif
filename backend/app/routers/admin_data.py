...
# ── Manuel Trust Score Tetikleyici ───────────────────────────────────────────

@router.post("/trigger/trust-scores", summary="Trust score hesaplamasını manuel başlat")
async def trigger_trust_scores(
    current_user: User = Depends(check_admin_access),
):
    """
    compute_trust_scores_task işini ARQ kuyruğuna koyar.
    Normalde her gün 02:15'te otomatik çalışır; bu endpoint gerektiğinde
    anında tetiklemek için kullanılır.

    - Sadece admin kullanıcısı çağırabilir.
    - 5 dakikalık Redis kilidi ile çift tetikleme önlenir.
    """
    from app.core.task_queue import get_pool

    LOCK_KEY = "admin:trust_score_lock"
    LOCK_TTL = 300  # 5 dakika
    ...
    if not acquired:
        ...
        raise BadRequestException(
            code="TRUST_SCORE_ALREADY_RUNNING",
            message=f"Trust score hesaplaması zaten kuyruğa alındı. {ttl} saniye sonra tekrar deneyin.",
        )
    ...
    if not pool:
        ...
        raise BadRequestException(
            ...
            message="ARQ worker pool henüz hazır değil. Lütfen birkaç saniye sonra tekrar deneyin.",
        )

    job = await pool.enqueue_job("compute_trust_scores_task")

    logger.info(
        "[Admin] Trust score manuel tetiklendi | admin=%s job_id=%s",
        current_user.email,
        job.job_id if job else "N/A",
    )

    return {
        "status": "queued",
        "job_id": job.job_id if job else None,
        "message": "compute_trust_scores_task kuyruğa alındı. İşlem birkaç dakika sürebilir.",
    }