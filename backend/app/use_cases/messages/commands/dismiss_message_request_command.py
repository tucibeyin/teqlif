...
class DismissMessageRequestCommand:
    """Pending mesaj isteğini hard-delete eder — thread silinir, relationship null'a döner.

    Decline'dan farkı: gönderen tekrar mesaj isteği gönderebilir.
    """
    ...
    async def execute(self, uid: int, requester_id: int) -> None:
        ...
        # Thread silinince unread request sayacını düşür
        redis = await get_redis()
        await redis.decr(f"msg:unread:request:{uid}")

        # Commit sonrası recompute — thread null olduğu için state "no thread" döner
