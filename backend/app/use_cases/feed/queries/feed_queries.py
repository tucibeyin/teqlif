...
class FeedQueries:
    ...
    async def get_personalized_feed(self, 
        ...
        if user_id:
            ...
            listing_ids = await self._popular_feed(offset, PAGE_SIZE, )
        ...
        if user_id and listing_ids:
            ...
            if my_listing_ids:
                imp_result = await self.uow.session.execute(
        ...
        if user_id and result:
            try:
                await self._mark_impressions(user_id, [r["id"] for r in result])
    ...
    async def _score_and_rank(self,
        ...
        try:
            redis = await get_redis()
            seller_res = await self.uow.session.execute(text(
    ...
    async def _mark_impressions(self, user_id: int, listing_ids: list[int],) -> None:
    ...
    async def get_foryou_feed(self, user_id: int, page: int,) -> list[dict]:
        ...
        badge_map_fy, trending_cats_fy, trending_lids_fy, trust_map_fy, influence_map_fy = await _fetch_seller_meta(foryou_uids)
        ...
        if organic_ids:
            try:
                await self._mark_impressions(user_id, organic_ids)
            except Exception as exc:
    ...
    async def _compute_foryou_ids(self, user_id: int, limit: int) -> list[int]:
        """
    ...
    async def _get_user_top_categories(self, user_id: int, n: int = 5) -> list[str]:
    ...
    async def _get_sponsored_listings(self,
        ...
        if user_id:
            try:
                user_cats = set(await self._get_user_top_categories(user_id, ))
            except Exception:
                pass
    ...
    def _inject_ads(self, organic: list[dict], ads: list[dict]) -> list[dict]:
        """
    ...
    def _inject_at_slots(self, organic: list[dict], items: list[dict], slots: list[int]) -> list[dict]:
    ...
    async def invalidate_user_feed_cache(self, user_id: int) -> None:
